import { Clock, EllipsisVertical, FilePenLine, MapPin, MessageSquare, ShieldCheck, Trash2 } from "lucide-react";
...
const UserWeekSearchItem = ({ avatar, weekSearch, profileId }) => {
   ...
   const handleMouseEnter = () => setIsActiveMenuOpen(true);
   const handleMouseLeave = () => setIsActiveMenuOpen(false);
   ...
   const handleUpdate = async (is_active) => {
      ...
      if(Number.isNaN(is_active)) {
         res = await updateUserWeekSearch(weekSearch.id, text, is_active = weekSearch.is_active);
      } else {
         res = await updateUserWeekSearch(weekSearch.id, text, is_active);
      }
      if(res) {
         router.replace(router.asPath);
      }
            ...
            <div className="ml-auto flex">
               <div className="relative flex flex-col">
                  ...
                  >
                     ...
                     <span className={`text-sm font-semibold ${weekSearch.is_active ? "text-green-500" : "text-red-500"}`}>
                  ...
                  {isActiveMenuOpen && (
                     <div
                        className="absolute right-0 top-9 mt-2 border w-[125px] bg-white rounded-lg shadow-lg z-10 overflow-hidden"
                        onMouseEnter={handleMouseEnter}
                        onMouseLeave={handleMouseLeave}
                     >
                        {weekSearch.is_active
